...
class StateTable:
    def __init__(self):
        self.table = [
            ...
            [60, 63, 2, 4], [60, 63, 2, 4], [62, 1, 5], [62, 65, 1, 5],
...
class SmartCompressor:
    ...
    def reverse_transform_01(self, data):
        return selfomics_1(data)  # Self-inverse
    ...
    def compress_with_best_method(self, data, filename: str):
        # For JPEG files (.jpg, .jpeg), force transform_07
        if is_jpeg_file(filename):
            logging.info("JPEG file detected. Forcing transform_07.")
            transformed = self.transform_07(data)
            best_compressed = transformed
            best_marker = 7
            best_method = 'paq' if self.paq_compress(transformed[1:]) else 'zlib'
            best_size = len(transformed)

            # Still check Huffman for small files
            ...
            # Original behavior for non-JPEG files
            transformations = [
                (1, self.transform_04),
                (2, self.transform_01),
                (3, self.transform_03),
...
def main():
    ...
    print("Options:")
    print("1 - Compress file (PAQJP_1 with transform_07 for JPEG)")
    print("2 - Decompress file (PAQJP_1 with all transformations)")
